...
class ConflictMap extends React.Component {
	constructor(props) {
		...
		let mapHeight = Config.conflictSettings[this.props.shortConflictSlug].mapHeights[this.props.breakpoint] + 'px';
		...
		this.state = {
			center: Config.conflictSettings[this.props.shortConflictSlug].centers[this.props.breakpoint],
			zoom: [Config.conflictSettings[this.props.shortConflictSlug].zooms[this.props.breakpoint]],
			style: Config.conflictSettings[this.props.shortConflictSlug].mapStyle[this.props.lang],
			selectedFeatures: [],
			interactable: false,
	...
	updateMarkers() {		
		...
		for (var i = 0; i < features.length; i++) {
			...
			if (!props.cluster) {
				...
				if(props.multiple_strike_targets === true && this.props.currentMap === 'strike-target'){
					id = props.post_id;
					marker = markers[id];
					if (!marker) {			
	...
	render() {
		...
		if(basicClusterSlugs.indexOf(this.props.shortConflictSlug) !== -1){
		...
		if (mapType === 'heat')  {
			...
			clustersLayer = <ConflictMapClusterLayers map={this.state.mapBoxMap} key={'cluster_'+this.props.currentMap} conflicts={this.props.conflicts} timelineStart={timelineStart} timelineEnd={timelineEnd} currentMap={this.props.currentMap}/>;
		}
		...
		return (
			<div ref="map-container" className="map-container" onMouseEnter={this.handleMouseEnter} onMouseLeave={this.handleMouseLeave}>
				...
				<Map			
					...
					scrollZoom={false}		
					style={this.state.style+'?fresh=true'}
