...
class CoalitionDeclaredStrikesTimeline extends React.Component {
	...
	render() {
		...
		let strikesToPointOnTimeline = this.props.data.timeline.slice(0, this.state.sliderValue);

		let startDate = moment(this.props.data.timeline[0].date, 'YYYY-MM-DD').format('MMMM DD, YYYY');
		let endDate = moment(this.props.data.timeline[this.props.data.timeline.length-1].date, 'YYYY-MM-DD').format('MMMM DD, YYYY');

		const timelineBars = [];

		//let maxStrikes = 0;
		this.props.data.timeline.forEach((strike, idx) => {
	...
	handleSliderChange(value, source) {
		if (value > this.props.data.timeline.length - 1) {
			value = 0;
		} else if (value < 0) {
			value = this.props.data.timeline.length - 1;
		}
