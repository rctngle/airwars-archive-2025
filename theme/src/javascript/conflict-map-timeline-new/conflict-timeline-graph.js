...
class ConflictTimelineGraph extends Component {
	...
	buildDays(){
		...
		this.props.conflicts.forEach(conflict => {
			...
			conflict.civcas_incidents.forEach(incident => {
				...
				if(days[day]){
					if(typeof incident.civilians_killed_min === 'number'){
						days[day].civilians_killed_min += incident.civilians_killed_min;
					}
					if(typeof incident.militants_killed_min === 'number'){
						days[day].militants_killed_min += incident.militants_killed_min;
					}
					if(typeof incident.num_incidents === 'number'){
						days[day].num_incidents++;
					}
	...
	render() {
		...
		if(this.props.conflictSlug.indexOf('us-forces-in-yemen') !== -1){
			...
			let monitoredStylesYemen = {
				left: '0%',
				width: '22.3%'
			};
			timelineBlockYemen = <div className="timeline-block monitored" style={monitoredStylesYemen}></div>
		}
				...
				<div className="timeline-container content">
					...
					<div className="full">					
						...
						<div className="overflow">
							...
							{timelineBlockYemen && timelineBlockYemen}
						...
						<Range 
							...
							defaultValue={[defaultMin, defaultMax]} handle={(props) => {
								...
								const date = getDateFromValue(this.props.dateRange, value-1);
								let dateFormatted = date.startOf('day').format('DD MMM YYYY');
