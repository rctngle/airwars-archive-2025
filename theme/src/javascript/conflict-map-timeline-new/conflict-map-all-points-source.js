	...
	render() {
		...
		this.props.conflicts.forEach((conflict) => {
			conflict.civcas_incidents.forEach((incident, i) => {
				...
				if (this.props.currentMap === 'civilian-fatalities' && (incident.civcas === undefined || incident.civcas === true)) {
					include = true;
					...
					include = true;
				} else if (this.props.currentMap === 'strike-target' && incident.targeted_belligerents !== undefined) {
					include = true;
				} 
